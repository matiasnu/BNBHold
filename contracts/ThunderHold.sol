...
contract ThunderHold {
    ...
    uint256 public constant INVEST_MAX_AMOUNT = 100000 ether; // 100.000 TT
    uint256 public constant INVEST_MIN_AMOUNT = 500 ether; //500 TT
    uint256[] public REFERRAL_PERCENTS = [40e2, 20e2, 10e2, 5e2, 5e2]; //4%,2%,1%,0.5%,0.5%
    uint256 public constant PROJECT_FEE = 50e2;
    uint256 public constant MARKETING_FEE = 50e2;
    ...
    uint256 internal constant LOTTO_TICKET_LIMIT = 50; // 50 Entrys Lotto
    uint256 internal constant LOTTO_USER_LIMIT = 10; // 10 Entrys Lotto per user
    ...
    function lottoDeposit(uint256 ticketsQuantity) external payable {
        ...
        lottoCurrentTicketsCount = lottoCurrentTicketsCount.add(
            ticketsQuantity
        );
        user.lottoparticipations = user.lottoparticipations.add(
            ticketsQuantity
        );
        ...
        for (uint256 i = 1; i <= ticketsQuantity; i++) {
            nlotto.currentLotto.push(
                cLotto(msg.sender, lottoLastTicket.add(1))
            );
            lottoLastTicket++;
        }
        ...
        uint256 value =
            uint256(
                uint256(
                    keccak256(
                        abi.encode(
                            block.timestamp.mul(mud),
                            block.difficulty.mul(mud)
                        )
                    )
                )
                    .mod(A)
            )
                .add(B); // Se resuelve con el resto y agregandole uno, el hash es aleatorio gracias al abi.encoded del timestamp
    ...
    function payLottoWin() internal {
        ...
        for (uint256 i = 0; i < LOTTO_TICKET_LIMIT; i++) {
            ...
            if (nlotto.currentLotto[i].ticketNumber == win1) {
                ...
                payable(address(uint160(nlotto.currentLotto[i].lottoId)))
                    .transfer(profit);
                users[nlotto.currentLotto[i].lottoId].lottobonus = (
                    users[nlotto.currentLotto[i].lottoId].lottobonus
                )
                    .add(profit);
    ...
    function getUserLottoRate(address userAddress)
        public
        view
        returns (uint256)
    {
        ...
        )
    {
        return (
            ...
            lottoLastWin5a,
            lottoCycles,
            lottoCurrentTicketsCount,
            lottoTotalTicketsCount,
            LOTTO_TICKET_LIMIT,
            LOTTOTICKET,
            lottoCurrentPot,
            lottoBag
    ...
    function getUserlottoStats(address userAddress)
        public
        view
        returns (
            uint256,
